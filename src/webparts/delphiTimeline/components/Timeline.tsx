...
export default class Timeline extends React.Component<ITimelineProps, ITimelineState> {
  ...
  public componentDidMount(): void {
    this.TimelineService.getTimelineActivities(this.props.listName, this.props.sortOrder).then((activities: any[]) => {
      const active  =  activities.map(x=> { 
        x.activityPictureUrl = JSON.parse(x.activityPictureUrl);
        return x;
      })
      this.setState({ timelineActivities: active });
    }).catch((error: any) => {
